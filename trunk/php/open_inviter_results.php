					...
					if ($inviter->showContacts())
					{
						...
						foreach ($_POST as $key=>$val)
						{
							if (strpos($key,'check_')!==false)
							{
								...
								$stringToUse2 = 'buttonOpenInviter';
								$stringToUse3 = 'contactNameOpenInviter';
								$stringToUse .= $counterForMe;
								$stringToUse2 .= $counterForMe;
								$stringToUse3 .= $counterForMe;
								$tempString = 'emailAdd2';
								$selected_contacts[$_POST['email_'.$val]]=$_POST['name_'.$val];
								$_SESSION[$stringToUse] = $_POST['email_'.$val];
								$_SESSION[$stringToUse3] = $_POST['name_'.$val];
								echo '<input type="button"  id=',$stringToUse2,' name=',$stringToUse2,' value="X" onClick="removeEmailOpen(',$tempString,',',$stringToUse,',',$stringToUse2,');"><input id=',$stringToUse,' name=',$stringToUse,' type="text" spellcheck="false" value=',$_POST['email_'.$val],'>';
								$counterForMe++;	
							}	
...
if (!$done)
	...
	if ($step=='get_contacts')
		{
		$contents.= "Login here to Import Contacts!";
		...
		$contents.="</select></td></tr>
			<tr class='thTableImportantRow'><td colspan='2' align='center'><input class='thButton' type='submit' name='import' onclick='removeFormClose();' value='Import Contacts'></td></tr>
	...
	if ($step=='send_invites')
		...
		if ($inviter->showContacts())
			...
			if (count($contacts)==0)
				...
				{
				$contents.="<tr class='thTableDesc'><td><input type='checkbox' onChange='toggleAll(this)' name='toggle_all' title='Select/Deselect all' checked></td><td>Name</td>".($plugType == 'email' ?"<td>E-mail</td>":"")."</tr>";
				$odd=true;$counter=0;
				...
				$contents.="<tr class='thTableFooter'><td colspan='".($plugType=='email'? "3":"2")."' style='padding:3px;'><input type='submit' name='send' value='Add E-Mail Address' class='thButton'></td></tr>";
				}
			$contents.="</table>";
			}
...
$contents.="</form>";
$contents.="<center><a href='http://openinviter.com/'><img src='http://openinviter.com/images/banners/banner_blue_1.gif?nr=68611' border='0' alt='Powered by OpenInviter.com' title='Powered by OpenInviter.com'></a></center>";
echo $contents;
?>